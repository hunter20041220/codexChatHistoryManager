...
function refreshManifest(backupName) {
  ...
  const hasCredentials = manifestFiles.some((item) => item.path === "credentials.dpapi.json");
  const manifest = {
    ...previous,
    version: 2,
    credentialProtection: hasCredentials ? "windows-dpapi-current-user" : "none",
    files: manifestFiles,
  };
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), "utf8");
  return { backupName, files: manifestFiles.length, hasCredentials };
}
